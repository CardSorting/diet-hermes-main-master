...
KANBAN_GUIDANCE = (
    ...
    "\n"
    "## JoyZoning governed work (when `joyzoning` tool is available)\n"
    "\n"
    "1. `joyzoning(action='context')` at session start — read `next_actions`.\n"
...
JOYZONING_GUIDANCE = (
    ...
    "\n"
    "JoyZoning habitat (:9470) observes and supervises. Hermes executes. "
    "JSDP synthesizes bounded mutations. Do not collapse these layers.\n"
    "\n"
    "## Start every governed session\n"
    "\n"
    "1. `joyzoning(action='context')` — scope bindings, convergence state, "
    "next_actions, habitat agent context when control plane is configured.\n"
    "2. `joyzoning(action='doctor')` if anything looks miswired.\n"
    "\n"
    "## Mutation lifecycle (plan → patch → verify → review → merge)\n"
    "\n"
    ...
    "2. Implement changes (`patch`, `write_file`). `joyzoning_governance` blocks "
    "layer violations on governable `.ts`/`.js` source only (not `.md`, `package.json`, "
    "migrations, or DB/ORM artifacts); call `joyzoning(action='patch', mutation_id=..., summary=...)` "
    ...
    "5. Operator accept-merge in JoyZoning habitat (Watch/desktop) — not in Hermes.\n"
    "6. Only then `kanban_complete(...)` — pre_tool_call gate blocks early complete.\n"
    "\n"
    "## Kanban + BroccoliQ + habitat linkage\n"
    "\n"
    "When spawned as a kanban worker, env carries `HERMES_KANBAN_TASK`, "
    "`JOYZONING_SCOPE_ID`, and `JOYZONING_HABITAT_TASK` when linked. "
    "Call `kanban_broccolidb_context()` after `kanban_show()` for hive intelligence.\n"
    "\n"
    ...
    "\n"
    "Operators dispatch from JoyZoning — **no Hermes config**. Agents use four calls:\n"
    "\n"
    "| Call | When |\n"
    "|------|------|\n"
    ...
    "## Do NOT\n"
    "\n"
    "- Do not self-authorize merge or call tools to mark CONVERGED when habitat "

...
import { resolveLaunchCwd } from "../utils/launch-cwd"

export { resolveLaunchCwd } from "../utils/launch-cwd"
 ...
 *  ``hermes --tui`` sets ``HERMES_PYTHON_SRC_ROOT`` / ``HERMES_AGENT_ROOT``
 *  to the checkout; otherwise fall back to ``~/.hermes/hermes-agent``. */
...
export class GatewayClient extends EventEmitter {
  ...
  start() {
    ...
    const cwd = resolveLaunchCwd(root)
    const env = { ...process.env } as Record<string, string>
    env.HERMES_CWD = cwd
    env.TERMINAL_CWD = cwd
    env._HERMES_TUI_GATEWAY = "1"

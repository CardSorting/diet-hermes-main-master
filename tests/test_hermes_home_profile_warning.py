...
profile is active, get_hermes_home() should:
  1. STILL return ~/.dietcode (raising would brick 30+ module-level callers)
...
class TestGetHermesHomeProfileWarning:
    def test_classic_mode_no_active_profile_no_warning(
        ...
        """Classic mode: no active_profile file → silent, returns ~/.dietcode."""
        result = fresh_constants.get_hermes_home()
        assert result == tmp_path / fresh_constants.PRODUCT_HOME_DIRNAME
        assert "HERMES_HOME fallback" not in capsys.readouterr().err

    def test_default_active_profile_no_warning(
        self, fresh_constants, tmp_path, capsys
    ):
        """active_profile=default → still no warning, returns native home."""
        hermes_dir = tmp_path / fresh_constants.PRODUCT_HOME_DIRNAME
        hermes_dir.mkdir()
        (hermes_dir / "active_profile").write_text("default\n")
        result = fresh_constants.get_hermes_home()
        assert result == hermes_dir
        assert "HERMES_HOME fallback" not in capsys.readouterr().err
    ...
    def test_hermes_home_set_suppresses_warning(
        ...
        root = tmp_path / fresh_constants.PRODUCT_HOME_DIRNAME
